...
VKLVolume DomainToVolume(const Domain &cloud, VKLDevice device, VKLVolume oldvolume) {
    ...
    float bkgrnd = VKL_BACKGROUND_UNDEFINED;
    VKLDevice dev = device;
    VKLVolume volume;
    if(!dev) {
        std::cout << "bogus device" << std::endl;
    }
    if(oldvolume==nullptr){
        std::cout << " creating new volume in tovkl " << std::endl;
        volume = vklNewVolume(dev,"structuredRegular");
    } else {
        std::cout << "using old volume " << std::endl;
        volume = oldvolume;
    }
    ...
    vklSetFloat(volume,"background",bkgrnd);
